# 12.1相机控制器CameraController的简单抽象
...
类属性方面，控制器持有其唯一的相机对象指针，持有平移速度、鼠标位置信息&灵敏度（用于之后的视角旋转方法）；

类公有方法中，有构造&析构、基向量及矩阵的计算、处理相机平移的方法、以及OnUpdate和OnEvent；

类私有方法中，有处理事件的一些方法，当触发OnEvent时，用事件方法器调用对应的方法，本次笔记中暂时没有用到。
